...
class Optimisationio_CdnRewrite
{
    ...
    {
        $this->cdn_root_url = $this->css_cdn_root_url = $this->js_cdn_root_url = untrailingslashit($this->get_setting('cdn_root_url'));
        if ($css_url = trim($this->get_setting('cdn_css_root_url'))) {
            $this->css_cdn_root_url = untrailingslashit($css_url);
        }

        if ($js_url = trim($this->get_setting('cdn_js_root_url'))) {
    ...
    {
        echo "<div class='update-nag'>Please add your CDN details if you wish to use the CDN functionality.</div>";
    }
